...
(function () {
  ...
  async function loadFromKintone() {
    ...
    try {
      const data = await kintoneApiCall('GET', 'records', {
        app   : APP_ID,
        query : 'order by 開始日 asc limit 500',
        fields: ['$id','現場名','開始日','終了日','開始時間','終了時間','担当者','作成者','備考','種別']
      });
      events = data.records.map(r => kintoneToEvent(r));
  ...
  async function loadApplicationEvents() {
    try {
      ...
      const data = await kintoneApiCall('GET', 'records', {
        app   : EXTERNAL_APP_ID,
        query : `非当番 in (${targetValues}) limit 500`,
        fields: ['$id','非当番','日付選択用','休みの形式','時間表示用','作成者']
      });
      console.log('[申請アプリ] 取得レコード数:', data.records.length);
      externalEvents = data.records
        .map(rec => {
